import { ENTITY_LABELS, FIELD_LABELS, STATUS_LABEL, vlanNumber } from "./fields";
import type {
  EntityKind,
  GraphEdge,
  GraphNode,
...
const SEARCH_FIELDS: (keyof InventoryRecord)[] = [
  "floor",
  "room",
  "userName",
  "nodeRow",
...
export function recordMatches(rec: InventoryRecord, rawQuery: string) {
  ...
  return rec.firewallAccess.some((fw) =>
    [fw.service, fw.protocol, fw.port, fw.destination, fw.action, fw.notes]
      .join(" ")
      .toLowerCase()
      .includes(q),
  );
...
function nodeId(kind: EntityKind, label: string) {
  return `${kind}:${label}`;
}

function pushNode(map: Map<string, GraphNode>, kind: EntityKind, label: string) {
  const trimmed = label.trim();
  if (!trimmed) return;
  const id = nodeId(kind, trimmed);
  const existing = map.get(id);
  if (existing) existing.count += 1;
  else map.set(id, { id, kind, label: trimmed, count: 1 });
}

function link(edges: GraphEdge[], set: Set<string>, a?: string, b?: string) {
  if (!a || !b || a === b) return;
  const key = `${a}|${b}`;
...
export function buildRelations(records: InventoryRecord[], query: string): RelationHit {
  ...
  for (const r of matched) {
    ...
    const node = r.nodeNumber
      ? nodeId("node", [r.nodeRow, r.nodeNumber].filter(Boolean).join("-"))
      : "";
    ...
    const iface = r.switchInterface
      ? nodeId("iface", `${r.switchName || "SW"}:${r.switchInterface}`)
      : "";
    ...
    pushNode(
      nodes,
      "iface",
      r.switchInterface ? `${r.switchName} ${r.switchInterface}` : "",
    );
    ...
    for (const fw of r.firewallAccess) {
      ...
      const fid = nodeId("firewall", label);
      link(edges, seen, ip, fid);
    }
  }
...
export function computeStats(records: InventoryRecord[]): InventoryStats {
  ...
  const group = (key: keyof InventoryRecord) => {
  ...
  const bySwitchMap = new Map<string, { value: number; active: number }>();
  for (const r of records) {
    const name = r.switchName.trim() || "نامشخص";
    const cur = bySwitchMap.get(name) ?? { value: 0, active: 0 };
    cur.value += 1;
...
export type TraceStep = { kind: EntityKind; label: string; hint: string };

export function buildTrace(rec: InventoryRecord): TraceStep[] {
  const steps: TraceStep[] = [];
  const add = (kind: EntityKind, label: string, hint = "") => {
    if (!label.trim()) return;
    steps.push({ kind, label: label.trim(), hint });
  };
...
export function generateSwitchportConfig(rec: InventoryRecord) {
  const vlan = vlanNumber(rec.vlan) || "1";
  const desc = [rec.floor && `F${rec.floor}`, rec.room, rec.userName || rec.computerName]
    .filter(Boolean)
    .join("-");
  ...
  if (rec.vlan.includes("VoIP") || rec.vlan.includes("۳۰") || rec.vlan.startsWith("30")) {
    lines.push(
...
export function uniqueValues(records: InventoryRecord[], key: keyof InventoryRecord) {
