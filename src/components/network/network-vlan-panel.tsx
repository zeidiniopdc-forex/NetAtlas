...
export function NetworkVlanPanel({ records }: { records: InventoryRecord[] }) {
  ...
  return <Card className="rounded-lg">
    ...
    <CardContent>
      ...
      <div className="mb-3 grid gap-2 sm:grid-cols-4"><Stat label="VLAN ثبت‌شده" value={vlans.length} /><Stat label="IP تخصیص‌یافته" value={totalAssigned} /><Stat label="VLAN فعال" value={vlans.filter((v) => v.status === "active").length} /><Stat label="Subnet" value={vlans.filter((v) => v.cidr).length} /></div>
...
function VlanForm({ value, busy, onChange, onSave, onCancel }: { value: Vlan; busy: boolean; onChange: (v: Vlan) => void; onSave: () => void; onCancel: () => void }) {
  const set = (key: keyof Vlan, nextValue: string | number) => onChange({ ...value, [key]: nextValue });
  return <div className="mb-4 rounded-md border border-border p-3"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2 text-sm font-medium"><Plus className="size-4" />{value.id ? "ویرایش VLAN" : "ثبت VLAN جدید"}</div><Button variant="ghost" size="icon" className="size-8" onClick={onCancel}><X className="size-4" /></Button></div><div className="grid gap-2 md:grid-cols-4">
    <Field label="VLAN ID"><Input type="number" min={1} max={4094} value={value.vlanId} onChange={(e) => set("vlanId", Number(e.target.value))} /></Field><Field label="نام VLAN"><Input value={value.name} onChange={(e) => set("name", e.target.value)} placeholder="Users" /></Field><Field label="Site"><Input value={value.site} onChange={(e) => set("site", e.target.value)} /></Field><Field label="ساختمان"><Input value={value.building} onChange={(e) => set("building", e.target.value)} /></Field><Field label="Purpose"><Input value={value.purpose} onChange={(e) => set("purpose", e.target.value)} placeholder="کاربران / سرورها / VoIP" /></Field><Field label="Subnet / CIDR"><Input dir="ltr" value={value.cidr} onChange={(e) => set("cidr", e.target.value)} placeholder="10.33.10.0/24" /></Field><Field label="Gateway"><Input dir="ltr" value={value.gateway} onChange={(e) => set("gateway", e.target.value)} placeholder="10.33.10.1" /></Field><Field label="وضعیت"><select className="h-9 w-full rounded-md border border-input bg-background px-3 text-sm" value={value.status} onChange={(e) => set("status", e.target.value)}><option value="active">فعال</option><option value="disabled">غیرفعال</option></select></Field><Field label="DHCP Start"><Input dir="ltr" value={value.dhcpStart} onChange={(e) => set("dhcpStart", e.target.value)} /></Field><Field label="DHCP End"><Input dir="ltr" value={value.dhcpEnd} onChange={(e) => set("dhcpEnd", e.target.value)} /></Field><Field label="یادداشت"><Input value={value.notes} onChange={(e) => set("notes", e.target.value)} /></Field>
...
function Field({ label, children }: { label: string; children: ReactNode }) { return <label className="space-y-1 text-xs"><span className="text-muted">{label}</span>{children}</label>; }
