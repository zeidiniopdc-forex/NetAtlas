...
const GROUPS: { title: string; keys: (keyof typeof EMPTY_RECORD)[] }[] = [
  { title: "موقعیت", keys: ["floor", "room", "userName"] },
  { title: "نود و پچ‌پنل", keys: ["nodeRow", "nodeNumber", "patchPanel", "patchPort", "patchRackPosition"] },
  { title: "سوئیچ و کابل", keys: ["switchName", "switchInterface", "cableNumber"] },
...
export function RecordForm({
  ...
  const [form, setForm] = useState<InventoryRecord>(() =>
    initial
      ? { ...initial, firewallAccess: initial.firewallAccess.map((f) => ({ ...f })) }
      : {
          ...EMPTY_RECORD,
          id: uid("rec"),
  ...
  const updateFw = (id: string, patch: Partial<FirewallAccess>) =>
    setForm((f) => ({
      ...f,
      firewallAccess: f.firewallAccess.map((row) => (row.id === id ? { ...row, ...patch } : row)),
    }));
        ...
        <div className="flex flex-col gap-3">
          {form.firewallAccess.length === 0 ? (
            <p className="rounded-md border border-dashed border-border px-3 py-4 text-sm text-muted">
              هنوز سرویسی ثبت نشده. با افزودن سرویس مشخص می‌شود این IP به چه مقصدهایی دسترسی دارد.
            </p>
          ) : (
              ...
              <div key={row.id} className="rounded-lg border border-border bg-surface-2 p-3">
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Select
                      value={row.protocol}
                      onValueChange={(v) => updateFw(row.id, { protocol: v as FirewallAccess["protocol"] })}
                    >
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Select
                      value={row.action}
                      onValueChange={(v) => updateFw(row.id, { action: v as FirewallAccess["action"] })}
                    >
                      <SelectTrigger className="h-9">
                        <SelectValue />
                      </SelectTrigger>
