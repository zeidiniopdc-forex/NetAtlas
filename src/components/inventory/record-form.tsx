...
const GROUPS: { title: string; keys: (keyof typeof EMPTY_RECORD)[] }[] = [
  {
    ...
    keys: ["site", "building", "floor", "room", "serverRoom", "rack", "userName"],
  },
  {
    title: "نود و پچ‌پنل",
    keys: [
  ...
  {
    title: "سیستم و شبکه",
    keys: ["computerName", "windowsUsername", "ip", "vlan", "network"],
  },
];

function normalizeRecord(initial?: InventoryRecord): InventoryRecord {
  ...
  const fw = Array.isArray(initial.firewallAccess)
    ? initial.firewallAccess.map((f) => ({
        id: f?.id || uid("fw"),
        service: f?.service ?? "",
...
export function RecordForm({
  ...
  const [form, setForm] = useState<InventoryRecord>(() => normalizeRecord(initial));

  // Keep form in sync when opening dialog for a different record
  useEffect(() => {
    setForm(normalizeRecord(initial));
  }, [initial?.id]);

  const set = (key: keyof InventoryRecord, value: unknown) =>
    setForm((f) => ({ ...f, [key]: value }));

  const updateFw = (id: string, patch: Partial<FirewallAccess>) =>
    setForm((f) => ({
      ...f,
      firewallAccess: (f.firewallAccess ?? []).map((row) =>
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        onSubmit({
          ...form,
          firewallAccess: Array.isArray(form.firewallAccess)
            ? form.firewallAccess
            : [],
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          ...
          <Select
            value={form.status}
            onValueChange={(v) => set("status", v as RecordStatus)}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between gap-2">
          <h4 className="text-xs font-medium text-muted">
            دسترسی‌های فایروال برای این IP
          </h4>
          <Button
            type="button"
            size="sm"
        ...
        <div className="flex flex-col gap-3">
          {(form.firewallAccess ?? []).length === 0 ? (
            <p className="rounded-md border border-dashed border-border px-3 py-4 text-sm text-muted">
              هنوز سرویسی ثبت نشده. با افزودن سرویس مشخص می‌شود این IP به چه
              مقصدهایی دسترسی دارد.
            </p>
          ) : (
                ...
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        updateFw(row.id, {
                          protocol: v as FirewallAccess["protocol"],
                        })
                      }
                    >
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        updateFw(row.id, {
                          action: v as FirewallAccess["action"],
                        })
                      }
                    >
                      <SelectTrigger className="h-9">
                ...
                <div className="mt-2 flex justify-end">
                  <Button
                    ...
                    onClick={() =>
                      setForm((f) => ({
                        ...f,
                        firewallAccess: (f.firewallAccess ?? []).filter(
                          (x) => x.id !== row.id,
                        ),
                      }))
                    }
